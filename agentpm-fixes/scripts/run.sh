...
# 4) Policy gates (keep a few quick ones) - WITH MOCK SUPPORT
[[ "${OPENAI_API_BASE}" == "http://127.0.0.1:1234/v1" ]] || [[ "${MOCK_LMS:-}" == "1" ]] || { 
  echo "OPENAI_API_BASE must be local (or set MOCK_LMS=1 for development)" >&2
  exit 1
}

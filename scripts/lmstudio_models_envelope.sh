...
if [[ "$count" -gt 0 ]]; then
    echo "{\"status\":\"success\",\"data\":{\"count\":$count,\"models\":$(echo "$models_response" | jq -c '.data[].id' 2>/dev/null || echo '[]')},\"error\":{\"message\":\"\"},\"meta\":{\"smoke_score\":0.0,\"reasons\":[\"models_found\"],\"scope\":[\"lmstudio_models\"],\"checks\":[\"count_ok\"],\"proofs\":[\"lm_studio_models\"]}}"
else

...
test.describe('PR-000 Simple UI Tests', () => {
  test('Basic UI load', async ({ page }) => {
    await page.goto('http://127.0.0.1:5173/ui/index.html');
    await expect(page.locator('body')).toBeVisible();
  });

  test('Help panel exists and can be toggled', async ({ page }) => {
    await page.goto('http://127.0.0.1:5173/ui/index.html');
  ...
  test('Health button fetches enveloped OK (server must be running)', async ({ page }) => {
    await page.goto('http://127.0.0.1:8700/agentpm/ui/');
    await page.click('#btn-health');
    const inspector = page.locator('#envelope-inspector');
    await expect(inspector).toContainText('"status": "ok"');
  });
});
